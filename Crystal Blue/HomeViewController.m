...
- (void)viewDidLoad
{
    if ([PFUser currentUser] && // Check if user is cached
        ...
        [query getObjectInBackgroundWithId:[PFUser currentUser].objectId block:^(PFObject *user, NSError *error) {
            if (!error) {
                NSLog(@"USER QUERY FETCHED");
                
                //PFUser *user = [PFUser currentUser];
                NSLog(@"valor modify: %ld",(long)[[user objectForKey:@"Modify"] integerValue]);
                if ([[user objectForKey:@"Modify"] integerValue]==1)
                {
                    [self performSelector:@selector(presentModifyScreen) withObject:nil afterDelay:0.1];
                }
            }
            else
            {
        ...
        [[PFUser currentUser] fetchInBackgroundWithBlock:^(PFObject *user, NSError *error)
         {
             if (!error) {
                 ...
                 if ([[user objectForKey:@"Modify"] integerValue]==1)
                 {
                     [self performSelector:@selector(presentModifyScreen) withObject:nil afterDelay:1];
                 }
             }
             else
             {
...
- (void)initializeInterface
{
    ...
    //Font Initialization
    [lab setFont:[UIFont fontWithName:@"AllerDisplay" size:42*(imageZoom-textZoom)]];
...
{
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"saveFacebook"]!=YES) {
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"saveFacebook"];
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Save your Game!"
                                                        message:@"Log in with Facebook in the pause view to save your game!"
                                                       delegate:self
...
- (IBAction)trophyButtonPressed:(id)sender
{
    ...
    actualUser[@"tXp"]=@([GameSaveState sharedGameData].totalXpGained);
    [actualUser saveEventually];
        ...
        [FBSession.activeSession refreshPermissionsWithCompletionHandler:^(FBSession *session, NSError *error)
         {
             if (!error)
             {
                 if ([FBSession.activeSession.permissions indexOfObject:@"user_friends"] == NSNotFound) {
                     ...
                     UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Facebook Permissions"
                                                                     message:@"Please allow the following permission to be able to see your friends scores!"
                                                                    delegate:self
    ...
    else
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Facebook Login"
                                                        message:@"Please sign in with Facebook in the pause screen."
                                                       delegate:self
                                              cancelButtonTitle:@"OK"
                                              otherButtonTitles:nil];
        [alert show];
...
{
    if ([alertView.title isEqualToString:@"Facebook Permissions"]) {
        [PFFacebookUtils reauthorizeUser:[PFUser currentUser]
                  withPublishPermissions:@[@"user_friends"]
...
- (IBAction)unwindToHome:(UIStoryboardSegue*)unwindSegue
{
    ...
    if ([GameSaveState sharedGameData].scrollToTheLeftLab==1) {
        [GameSaveState sharedGameData].scrollToTheLeftLab=0;
        [scroller scrollRectToVisible:CGRectMake(1400, 0, scroller.frame.size.width, scroller.frame.size.height) animated:YES];
    }
    ...
    [query getObjectInBackgroundWithId:[PFUser currentUser].objectId block:^(PFObject *user, NSError *error) {
        if (!error) {
            ...
            else if ([[user objectForKey:@"Modify"] integerValue]==2)
            {
                user[@"Modify"]=@0;
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Gift!"
                                                                message:@"\nEnjoy some gold bars!\n"
                                                               delegate:self
                                                      cancelButtonTitle:nil
...
- (void)presentModifyScreen
{
     NSLog(@"Entro a MODIFY");
     NSString *storyboardName = @"Main";
     UIStoryboard *storyboard = [UIStoryboard storyboardWithName:storyboardName bundle:nil];
     UIViewController *fbl = [storyboard instantiateViewControllerWithIdentifier:@"modifyScreen"];
     fbl.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
     [self presentViewController:fbl animated:YES completion:nil];
}
